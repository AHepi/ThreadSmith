...
Not in either bundle, and so not here: the authority document itself (file 10, and its revised version 20), the other model's outputs (its amendments, audit package, Stage A and Stage B tables, rulebook audit V3, literary stress test package), and the earlier numbered files the read-me lists as superseded and left out on purpose. Each project's INDEX marks these.
