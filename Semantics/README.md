...
- **Live instruction:** [S63](<tests/S63 Next instruction for the other model - ten near cases by an outside author, returned as a zip.md>): ten near cases by an outside author (O15 to O24) on the ten rows named, run through the card, tighter pair, fix card and all-cases routine; coverage updated; returned as one zip. Waits on the owner for file 20 and the first 32 Stage B rows.
...
tests/         the audit workflow pair (24) and the instruction rounds (27, 28, 30, 41, 55, 57 superseded unsent, S62, S63 live)
