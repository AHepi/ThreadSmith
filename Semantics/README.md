...
## Current state
- **Authority:** [file 10](<authority/10 Claude Fable Semantics - standalone theory.md>), in the repository since log 56. The revised version (file 20) was set aside by the owner as a regression (decision S4) and is kept out on purpose. The other model has audited 20 since log 26; from S64 on it is told to place every verdict under the earlier version and to mark what rests on 20 alone. See [authority/](authority/).
...
- **S62 returned** (log S63), held as [S62 Stage D and report - return/](<results/S62 Stage D and report - return/>), ten files unchanged. All 56 clause-by-case comparisons SAME; the four judgement rows unchanged; the top finding restated at its size; the revised theory's anchored grade printed beside file 10's argument that there is no anchoring condition; coverage 24 of 80 recovered rows, 32 rows unrecovered; ten rows named to strain first. **Version update: not yet.** Nothing forced a change, and a new version would have to be built from file 20, which is not here.
- **S64 returned** (log S65), held as [S64 Near cases - return/](<results/S64 Near cases - return/>). Of the ten near cases, five were placed by the rows' own pairs, three by tighter pairs, and two (rows 90, 96) stayed open with clauses of the model's own that name the judgement rather than replace it; six clauses by 24 cases, all SAME; coverage 34 of 80 recovered rows. The S62 findings sorted under file 10: the anchored grade, attack F, the adequacy pass, the merit gate and their Part XV entries fall away as REVISED ONLY; the contract distinction, the table sentence, and the scope, question and contribution limits stand. **Version update: still unearned.**
- **Live instruction:** [S65](<tests/S65 Next instruction for the other model - thirteen near cases, two attribution tests, returned as a zip.md>): thirteen cases by an outside author, O25 to O37: two histories that test H64, one assisted diagnosis that tests I64, ten near cases for the next ten rows; returned as one zip. Still owed by the owner: the first 32 Stage B rows.
...
tests/         the audit workflow pair (24) and the instruction rounds (27, 28, 30, 41, 55, 57 superseded unsent, S62, S63 superseded unsent, S64, S65 live)
results/       55, 57, S62 and S64 returns (unchanged); S65 the saved chat page's messages; NOT-IN-BUNDLE.md for the earlier returns, quoted in the log and not held here
```
