...
- **Live instruction:** [S62](<tests/S62 Next instruction for the other model - Stage D, clause tests, coverage and the report, returned as a zip.md>), which replaces [57](<tests/57 Next instruction for the other model - Stage D fix cards and the report.md>) (written, never sent). Stage D's four fix cards; each clause run on all fourteen cases; the top finding at its size; the two anchoring passages compared case by case; near-case coverage of the 112-row table; the theory's attack points and refutation entries walked; every quotation listed; the report; a manifest. Returned as one zip. Not yet sent.
...
tests/         the audit workflow pair (24) and the instruction rounds (27, 28, 30, 41, 55, 57 superseded unsent, S62 live)
