...
## Tiny demonstration
Open [Lessons](<../../Language/records/Checked reasoning language - Lessons.md>), entry 35:
...
Walk it back:
1. **The thing that failed:** the Stage B instruction in [30 Next instruction - workflow update and re-audit](<../tests/30 Next instruction for the other model - workflow update and re-audit.md>), "Add a judgement-phrase pass".
...
3. **Why it counts as a failure:** rounds 3 and 4 ([27](<../tests/27 Next instruction for the other model - round 3, outside cases.md>), [28](<../tests/28 Next instruction for the other model - round 4, two change-based clauses.md>)) had already shown that "appropriate to the intended question" and "independently grounded dependency" rest on judgement. A pass that clears both has not measured anything.

Walk it forward:
4. **The fix:** [41 Next instruction - finish Stage B and near cases](<../tests/41 Next instruction for the other model - finish Stage B and near cases.md>), Task 2: for each outside case already run, say which change of the pair it matches, and whether that placing agrees with the round's verdict.
...
## Where to find it
`records/` of the project, numbered. The log entry with the nearest number gives the fuller account.
