...
when on from s80_common.effort_for(purpose, provider), temperature pinned, max_tokens at the top rung of the provider's
reader ladder, s80_common.ladder_for), and one marker-shaped call per marker provider (thinking on, max_tokens at the
top rung of the provider's marker ladder, a JSON answer checked by parsing).
  python Semantics/tools/s80_probe.py [--purpose s80|audit] [--save DIR]
--purpose picks the effort map's row: "s80" (the default, S80's readers and markers, "high") or "audit" (S81 Stage 2
and cross-examination: Atria and Mimo "medium", DeepSeek "high"). Every call takes a provider slot
(s80_common.provider_slot), as every other call does.
...
def one(provider, thinking, max_tokens, system, user, want_json=False, purpose="s80"):
    body = build_body(provider, system, user, thinking, max_tokens,
                      effort=C.effort_for(purpose, provider) if thinking else None)
    t0 = time.time()
    try:
        with C.provider_slot(provider, label="probe"):
            t0 = time.time()
...
def main(a):
    rows, out = [], []
    purpose = a[a.index("--purpose") + 1] if "--purpose" in a else "s80"
    if purpose not in C.EFFORT:
        raise SystemExit("--purpose must be one of %s" % sorted(C.EFFORT))
    ...
    for p in C.MODELS:
        for th in (True, False):
            show(one(p, th, C.ladder_for(p, C.READER_LADDER)[-1], "You are reviewing a document.", "Say OK.",
                     purpose=purpose))
    for p in C.API_MARKERS:
        show(one(p, True, C.ladder_for(p, C.MARKER_LADDER)[-1], None,
