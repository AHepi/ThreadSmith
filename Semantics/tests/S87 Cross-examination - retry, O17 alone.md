...
**Your job is to try to overturn that determination.** This text carries one of the seven pressed rows, O17, one of the three rows test (b) turned on; in it, "the pressed row" means O17. The other six rows and the standing clause are examined in separate texts. Test its ruling on O17 against the texts as hard as you can, and give its reasons their full weight. The testers, the determiner, and the author of the theory, the revision, the cases and the fixed verdicts all belong to one model family. You read from outside it.
...
1. **The pressed row.** For O17, try to show from the texts that the ruling is wrong: that the file-10 mark, the file-11 mark, the verdict, the passage, the direction or the kind is not what the texts give under the rules in section 2. Quote file 11 with line numbers, and file 10 where it is excerpted. If you need file-10 text that is not excerpted, name it and say why. If you cannot overturn the ruling, say which argument you tried and why it failed. The question this row raises:
   - **O17:** does file 11 reach the fixed verdict on every point (AGREE, a qualification allowed under "Same finding"), or does it leave a point open (SILENT) or support two readings that reach different findings (SPLIT)? A file-11 SILENT or SPLIT on it is a theory change away.

**The form of your report.**
- Number your points, most serious first. After them, put one line that reads exactly "O17: RULING STANDS" or "O17: RULING FALLS — <mark>", with nothing else on that line. After the dash, give the file and the mark you would rule instead, for example "O17: RULING FALLS — file 11 SILENT". A ruling falls when the texts, under the rules in section 2, give a different mark under either file, or a different verdict, passage, direction or kind.
