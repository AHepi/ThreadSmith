# Revision 2 - worklist, draft of 23 September

**DRAFT — not frozen; decisions D1–D11 open**

*Moved into the repository on 23 September 2026 from the session scratchpad, where it was `revision2/01 worklist draft.md`. Edits on moving in: the title, the line above, this paragraph, and the pointers that led into the scratchpad. Written on 23 September 2026 by a Claude subagent: every known candidate change for a revision 2 of the theory (W1–W56), each with its place in files 10 and 11, its severity, its handling and its dependence on S81. It decides nothing. The checks it called for on W31, W19 and W20 are in `results/S88 Claude's checks of the three defects/`, and the plan built on it, whose section 4 lists the open decisions D1–D11, is `tests/Revision 2 - plan and test round, draft of 23 September.md`.*
...
- **Source audit and candidate cases (then in the session scratchpad):** `results/S89 The theory against its sources - Deutsch and Marletto.md` and `tests/S89 Case book - candidate cases N1 to N25 drawn from the sources.md`.
...
- **Book pages.** Every book page and quotation comes from the verification file. Claude did not reread the books. The text of the Pinker book was not opened.
