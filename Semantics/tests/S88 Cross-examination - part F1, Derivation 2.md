...
This text carries only **F1**; F2 and F3 are examined in separate texts. Your job is to test that claim as hard as you can against the theory's own text, and to judge the repair proposed for it.

The theory's full text comes first, word for word, between two marker lines. Then the finding follows. It is stated with its counter-instances, the arguments made in the theory's defence together with the replies to them, and the repair proposed. Nothing in the finding has been settled. Treat it as a claim to be tested, and give the theory's defence its full weight.

**Citing the theory.** Line numbers count the lines of the theory text, with its title line as line 1. Quote the theory's words whenever you rely on them. The theory's stated assumptions are what its own text states.
...
Notation. \(\mathrm{do}(x{=}c)\) is the edit that sets port \(x\) to the value \(c\); by line 120 it replaces the component that assigns \(x\).
...
- One section, headed **F1**.
- Within it, number the points, most serious first.
- Close the section with one line that reads exactly F1: UPHELD, or F1: PARTLY UPHELD, or F1: REFUTED, with nothing else on that line:
