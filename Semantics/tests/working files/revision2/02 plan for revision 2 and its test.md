# 02 Plan for revision 2 of the theory, and the round that would test it
...
**Read.**
- The worklist `revision2/01 worklist draft.md` (W1–W56) and the three checks in `revision2/checks/`.
...
- `sources/verify/01 verification of source observations.md` and `source_cases/cases - final candidates.md`.
...
- **Provenance.** For each new case the provenance file gives:
  ...
  - the sketch's aim, from `sketches - why each was chosen.md`, which travels in no brief.
