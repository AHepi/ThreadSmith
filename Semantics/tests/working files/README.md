...
These are working files made in the session scratchpad and cited by committed records. They were copied here on 25 September 2026 so that they are not lost when the temporary machine is cleared (lesson S23). Every file is unchanged, byte for byte, except three edited on 25 September: `errcorr/03 synthesis.md` and `errcorr/04 checked answer.md`, only to shorten quote chains to 25 words or fewer (decision S19), each with a dated line; and `rivals/03b model attack.md`, only to add one dated line giving the new md5 of `attack/a11_patch.py`, whose docstring quotation was shortened for the same rule. A file the records call `scratchpad/rivals/01 entries.md` is `rivals/01 entries.md` here. A file already in the repository with the same content was not copied again; each section below says where it is. All of them were made by Claude subagents or by the orchestrator.
...
- Already in the repository, so not here: every script and output under `attack/`, `attack3a/`, `final/` and `models/` that is in `tests/Revision 2 - rivals and problems - scripts/` (there, since 25 September, `attack/a11_patch.py` has a shorter book quotation in its docstring; nothing that runs changed); `base_theory.md` (draft 3's theory text); `file13_draft4_theory.md` and `final/try_theory.md` (draft 4's theory text); `final/cl_draft4.md` and `final/note_draft4.md` (the change list and the revision note, in `tests/`); `f1_repo_run.txt` (the scripts folder's `final/f1_output.txt`).
