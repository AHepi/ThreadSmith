...
**Approximate transport.** With one-step discrepancy \(\varepsilon\) and an \(L\)-Lipschitz next-step map, \(e_n\le\varepsilon L^{n}\). (T2) Without a modulus, no accumulated bound follows. An exact question is not silently replaced by an approximate one.
...
(P) does not rank alternatives. Losses outside \(P\) must be exposed. \(\operatorname{ProducedBy}(\Delta,\xi,\xi';O)\) holds when a correct account of the repaired obligation is among the results of \(\Delta\) and is deployable at \(\xi'\); whether the change from \(\xi\) to \(\xi'\) ran through that account is a question about histories (Part IX) and is not part of (P).
