...
This text carries only **F3**; F1 and F2 are examined in separate texts. Your job is to test that claim as hard as you can against the theory's own text, and to judge the repair proposed for it.

The theory's full text comes first, word for word, between two marker lines. A short passage from the document the theory was condensed from comes next. Then the finding follows. It is stated with its counter-instances, the arguments made in the theory's defence together with the replies to them, and the repair proposed. Nothing in the finding has been settled. Treat it as a claim to be tested, and give the theory's defence its full weight.

**Citing the theory.** Line numbers count the lines of the theory text, with its title line as line 1. Quote the theory's words whenever you rely on them. The theory's stated assumptions are what its own text states. The predecessor passage is context: it shows what the earlier document said, and it is not an assumption of the theory under audit.
...
## A passage from the theory's predecessor

The theory was condensed from an earlier, longer document. That document is not under audit. One of its passages is quoted here word for word because the finding refers to it: passage 2 is its version of non-circular dependence. Passage 1, on approximate transport, belongs to another finding and is left out.
...
Notation. "S1", "S2" and "S3" name the three sentences of the non-circular dependence condition at line 270, in order.
...
2. **Judge the proposed repair.** Does it fix the defect? Does it break anything else? Name each case or derivation it breaks, with the reason. Check at least Derivations 1, 8, 9 and 10, Part VI, the worked cases of Part VII, the paragraphs "What (E) excludes" and "What (E) does not exclude", and O33 and O5. If you would repair differently, give your wording.

**Report format.**

- One section, headed **F3**.
- Within it, number the points, most serious first.
- Close the section with one line that reads exactly F3: UPHELD, or F3: PARTLY UPHELD, or F3: REFUTED, with nothing else on that line:
