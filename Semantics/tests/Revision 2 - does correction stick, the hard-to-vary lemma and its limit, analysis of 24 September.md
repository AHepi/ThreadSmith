...
**Source check (Deutsch, for context).** [T] "the harder it is to vary an explanation, the harder it is in particular to construct a variant with a different reach …" (D p.29). **[CHANGED for the repository: quotation shortened to 25 words or fewer.]** "You cannot confine its predictions to a region of your choosing" (D p.28). The confined variant "is no longer an explanation of seasons, just a (purported) rule of thumb" (D p.28). "its defenders would have had nowhere to go" (D p.25). [F] The theory matches the first two in fact: reach is fixed by E and the world, and a narrowing changes the job claimed, not E's reach. It matches the third only in part: no interventions means no production question (D3:L151), and uniform seen cases fail non-circular dependence. It lets a confined narrowing bundled with a fitted patch pass (E).
