...
- **Written** 23 September 2026. This file is one of three texts cut from one of three parts of a brief of that date, one for each of that part's pressed rows, and it is sent, as the one message, to one auditor. No other reply, to that brief, to another part or to another of these texts, is shown with it. Nothing in it was sent before it was committed.
- **Sources**, each checked by md5 when this file was built:
  ...
  - the case book (md5 4f488d149e44669240d5db546c8e946a), whose case for the pressed row in this text is given word for word in section 5;
...
- **How the replies will be read.** This rule was written and committed before this file was sent.
  ...
  The reply to this text is read by this rule on its own, independently of every other reply: the replies to the brief, to its parts and to the two other texts cut with this one, and the other auditor's reply.
- **Length:** 15,134 words.
...
**Your job is to try to overturn that determination.** This text carries one of the seven pressed rows, O30, one of the three rows test (b) turned on; in it, "the pressed row" means O30. The other six rows and the standing clause are examined in separate texts. Test its ruling on O30 against the texts as hard as you can, and give its reasons their full weight. The testers, the determiner, and the author of the theory, the revision, the cases and the fixed verdicts all belong to one model family. You read from outside it.

**What this brief holds.**
- Section 2: the rules the determination applies, word for word.
- Section 3: file 11 in full.
- Section 4: file 10, **excerpted**: only the passages the pressed row rests on, their counterparts, and every file-10 sentence the determination cites on that row.
- Section 5: the pressed row, with its case, the determination's ruling and reasons, the contrary readings, and the marks.
...
**File 10 is excerpted.** Below are only the file-10 passages the pressed row rests on, the file-10 counterparts of the file-11 sentences that row rests on, and the full text of every file-10 sentence the determination cites on that row. Each line is given whole, with its file-10 line number and the heading it stands under. Where file 11 has a sentence with no counterpart, this section says so. The excerpts that only other pressed rows need have been cut. For any other file-10 text, name it in your reply and say why you need it.

**Counterparts.** Where a file-11 sentence the pressed row rests on has a file-10 counterpart, it is excerpted below. These file-11 sentences have **no counterpart** in file 10:
...
Extended in file 11: f10 L414 = f11 L401 s1–s3, and file 11 adds sentences.
...
For the row: the case, word for word; the marks of the testers and auditors (marks only); the determination's ruling and its reasons, quoted; the contrary readings; and what turns on the row. In the marks line, "own" is YOUR MARK and "blind" the BLIND MARK, each in the order auditor 1 on A, auditor 1 on B, auditor 2 on B. Auditor 2 on A is missing on every row.

This row is one of the three (O5, O30, O17) that decide test (b). On each, the file-11 mark is ruled AGREE against part of the readers. If the file-11 mark were SILENT, the row would be AGREE→SILENT through new sentences of file 11: a theory change away from the thoughtful person, and test (b) would fail.
...
Work from the texts: file 11 in section 3, file 10 as excerpted in section 4, and the case. The marks of the testers and auditors are evidence, as they were for the determination; a count of readers settles nothing. Where a ruling holds, say so briefly and move on.

1. **The pressed row.** For O30, try to show from the texts that the ruling is wrong: that the file-10 mark, the file-11 mark, the verdict, the passage, the direction or the kind is not what the texts give under the rules in section 2. Quote file 11 with line numbers, and file 10 where it is excerpted. If you need file-10 text that is not excerpted, name it and say why. If you cannot overturn the ruling, say which argument you tried and why it failed. The question this row raises:
   - **O30:** does file 11 reach the fixed verdict on every point (AGREE, a qualification allowed under "Same finding"), or does it leave a point open (SILENT) or support two readings that reach different findings (SPLIT)? A file-11 SILENT or SPLIT on it is a theory change away.

**The form of your report.**
- Number your points, most serious first. After them, put one line that reads exactly "O30: RULING STANDS" or "O30: RULING FALLS — <mark>", with nothing else on that line. After the dash, give the file and the mark you would rule instead, for example "O30: RULING FALLS — file 11 SILENT". A ruling falls when the texts, under the rules in section 2, give a different mark under either file, or a different verdict, passage, direction or kind.
- Keep the report under about 2,000 words. Depth on the points that decide the ruling counts for more than coverage of small points.
