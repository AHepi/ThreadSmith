...
Every rule below but one was used in one project's record before it was written here; the exception is the Opus 5 permission in `references/owner.md`, which came from decision H7 after this file was drafted. Used is not tested: the record used these rules on one skill under test, read by two models and marked by one marker pair, and on nothing else. Where a rule came from is in `references/`.

## The loop
1. **Read the record first.** The read-me, the status, the newest numbered file. Never plan from memory of a record.
2. **Freeze the question.** One claim, one aim. What would count as the claim failing.
...
6. **Read the result under the frozen criteria only.** Counts first, and a rate only beside its count, never on a small n and never as a pass mark; no totals across arms; every claim checked against its source with every hit printed; predictions ticked one by one; what was not tested named.
7. **Decide what the result forces.** A change to the thing under test enters only when a test forces it, as one new numbered file, answering every open fault at once, in the fewest passages; then the same test again under the new file. A break seen once forces a repeat; what the repeat's outcome forces is written into the plan before the repeat runs. See `references/changes.md`.
...
- **A result with n of one.** It is an observation. Three per box is a repeat, better than one and still small: "0 of 6" is compatible with a fault that fires one time in ten.
...
| choosing cases from research, or building a home-made test rig | `references/testing-against-cases.md` (on loan from the hard-to-vary skill) |
