...
- **One sequence, three projects.** The numbers were one sequence across the projects while they shared one record, so a project's numbers have gaps where another project's entries fall. Since the record was split, a new entry carries the project's letter and the shared sequence's next number at the time of writing (for example S60, L60, H60), and a file made under it is named the same way ("S60 ..."). New decisions and lessons in a project's own record start at 1 with the letter. The letter says the project; the number says roughly when.
