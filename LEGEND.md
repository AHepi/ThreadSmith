...
- **Numbers 37, 38 and 39 were used twice** for a while: a later chat, starting without the files, reused them. Its three documents were renumbered 41, 42 and 43 before import, and the numbers inside them corrected (Lesson 37).
- **The kinds** that appear: Test plan (frozen before a run), Test results (what happened), Next instruction for the other model (an audit round), Workflow and Owner's guide (the audit pair), Skill, Prompt (a translator's task), Corpus (cases with answer keys), Checker rigs. Kinds named in the log but not in the bundles: Theory, Research plan, Research, Reference, Rulebook.
...
- `41 Next instruction for the other model - finish Stage B and near cases.md`: log 41 (the later chat); an audit round to hand to the other model; asks it to finish its Stage B table and hold each pair against a near case. The live one; 27, 28 and 30 are rounds already run.
