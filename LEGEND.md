...
## The record files
`Checked reasoning language - project story.md`, `- Decisions.md`, `- Lessons.md`, `- Status.md` carry no number. They are one record for all three projects up to log 59 (the name is the first project's, kept). Since 21 September 2026 `Semantics/records/` and `Language/records/` hold those projects' own copies of the same four files, with new entries lettered (S60, L60; decisions and lessons S1, L1). All are overwritten in place and only ever appended to. The **log** in the project story is the record; Status is the summary; Decisions holds the owner's words; Lessons holds only failures.
