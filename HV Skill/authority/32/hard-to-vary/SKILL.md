...
**Step 2 - List the jobs.** The particular things the explanation has to account for, each put as a contrast: why this *and not that*. Mark where each job came from: **given** (the owner named it), **fixed** (the owner made it a requirement that is not up for test), or **added** (you put it there). A part held only by a job you added is not held. It is *held if* that job is real, and the report must say so. ("Added" here is a tag on a job's provenance. The test called *Add a job* in Step 5 is a different act: proposing a further thing that must also be so and checking it. Writing the tag does not run the test.)
...
Two vocabularies, never mixed. The eight marks (Step 3's list) say what holds a part in place now; the provenance words (fitted, built, asserted) say where it came from. Every part gets one of each. A provenance word is never written in the mark column, and a mark is never written as a provenance: *asserted* is not a mark, and *borrowed* and *fixed* are not provenances.
...
- **Stop** when every part carries one of the eight marks: held (and by what; where two parts hold a job only together, say "held, jointly with" and name the other), held if (held only by a job, or only by another part, that is itself in doubt; name it), two routes (either of two parts does the job; name both), loose (and what could replace it), idle (removable), borrowed (held in place by some other explanation that is not being tested here; name it), fixed (a job or requirement the owner put outside the test, and said so; not a part the owner merely asserts, which is inside the test, has the provenance *asserted*, and takes whatever mark holds it; and not a free choice, which is loose), or unknown (and what test would settle it).
...
3. **Part by part:** one of the eight marks: held (by which job or change), held if (which doubtful job or part), two routes (which two parts), loose (what could replace it), idle (can be removed), borrowed (rests on another explanation; name it), fixed (a requirement the owner put outside the test; never a part the owner asserts, never a free choice), or unknown (what would settle it). A table is fine. Never add the marks up.
4. **Whole-explanation checks:** flip, direction, answer hidden in the starting points, where the jobs came from, add a job, look inside, pairs that pull, catch-alls and their gauges, what the change list leaves out, rivals, and where the parts came from. (Remove, swap and poke are run part by part and reported in section 3; a test with no purchase on a document of this kind is said to have none, not left out.)
