...
## Current state
- **Authority: [file 33](<authority/33/hard-to-vary/SKILL.md>), frozen 22 September 2026** (log H69, decisions H6 and H7): file 32 with every edit five Opus 5 judges armed with the skill found the results in hand to force, in thirty-five lines across six files: five word lists each in its place; the report names every test by its Step 5 name and puts provenance beside every mark; *held if* and *fixed* with one reading each; the poke's second change next to the disputed line; a swap loose only if every job is still done; a derived conclusion held by a named condition; a test that cannot bite named and its part left unknown. No change until the research plan from Semantics file 10 asks for one. Unpacked in `authority/33/`, archive beside it. No reader has run under it. [Draft file 32](<authority/32/hard-to-vary/SKILL.md>) (log H67) is the step between; [file 31](<authority/31/hard-to-vary/SKILL.md>), made under log H58 after the repeatability run (H57) forced it: file 30 with two changes (the flip is not for a derived conclusion; *fixed* is only for what the owner put outside the test). Unpacked in `authority/31/hard-to-vary/`, the archive `31 Skill - ... .skill` beside it. Tested in plan H58 (H59): neither break recurred in twelve runs, no new fault. [File 30](<authority/hard-to-vary/SKILL.md>), "modular, with router and map" (8 files, 13,614 words; log 43), stays in `authority/hard-to-vary/` with its archive: it is the version every test up to H57 ran on.
...
- **Hardening from the results in hand (H68, H69):** five Opus 5 judges, each armed with the skill and made to falsify its own findings, read the other projects (read only), checked the word list against Semantics file 10 row by row, judged the five changes of file 32 and every part of the skill, and hunted two-reading passages; every forced edit is in file 33; what they left loose or unknown is named in H69 and held.
