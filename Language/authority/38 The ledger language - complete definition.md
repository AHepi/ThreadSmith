...
- **NOT [E BECAUSE C]** is fine unless the ledger's own lines make C produce E.
...
A what-if names one change and asks whether some line would then be so.
...
Every report also carries: a note wherever a finding leans on a line the writer did not write; the count of lines said, filled in and usual case; the verbs with no shape; and the bin.
...
3. Every thing in a happening has a thing line. If the text does not say how it answers a press, write "not stated". A class you guess is a filled-in line.
