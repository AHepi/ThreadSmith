...
def check(ledger_path, log):
    ...
    # ---- PATCH 11 (pile 2, F21): what-ifs of two kinds. WITHDRAW: nobody said it. MAKE NOT SO: it did not happen.
    for w in meta.get("whatifs", []):
        if "withdraw" in w:
            result = bool(ask(w["ask"], removed=[w["withdraw"]])); how = "WITHDRAW line %s (as if nobody had said it)" % w["withdraw"]
        else:
            direct = sorted({r["lines"][0] for r in ask("holds(%s)" % w["make_not_so"]) if len(r["lines"]) == 1})
            result = bool(ask(w["ask"], removed=direct, extra="denied(%s).\n" % w["make_not_so"])); how = "MAKE NOT SO: %s (as if it did not happen)" % w["make_not_so"]
        report.append('WHAT-IF [%s]: "%s"\n  The change made: %s.\n  Your what-if %s.' % (w.get("whose", "?"), w["text"], how, "HOLDS" if result == w["claims"] else "FAILS"))
    ...
    if not any(k in r for r in report for k in ("CONTRADICTION", "JUMP", "KIND MISTAKE", "IDLE", "CIRCLE", "UNEXPLAINED", "DEPARTURE", "NO CONNECTION", "PULL OPPOSITE", "EXEMPTION FROM", "ADDED LINES ALONE")):
