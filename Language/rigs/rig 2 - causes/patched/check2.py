...
def say_direction(d):
    ...
    return (WORDS[m.group(1)] % m.group(2)).replace("the he","him").replace("the she","her") if m else {"across": "sideways", "down": "down", "up": "up"}.get(d, d)
...
def findings_for(ledger, meta, log, remove=(), swap=None):
    ...
    for a in ask(ledger, "said_moves(E, T, D)", log, remove, swap):
        ...
        if weak:
            ...
            kind = "direction"
        elif ask(ledger, "no_shape(E9, V, %s)" % thing, log, remove, swap):
            v = ask(ledger, "no_shape(E9, V, %s)" % thing, log, remove, swap)[0]["bind"]["V"]
            found.append((e, "not checked", head + "\n  NOT CHECKED. The verb '%s' is not in the shape book, so the checker cannot tell what kind of happening it is." % v)); continue
        elif ask(ledger, "press_unresisted(E9, I, %s, %s)" % (thing, d), log, remove, swap):
            found.append((e, "cannot tell", head + "\n  CANNOT TELL. Something does press it that way, but the ledger does not say how %s answers a press (its response is 'not stated')." % name(meta, thing))); continue
        elif ask(ledger, "press_unsized(E9, I, %s, %s)" % (thing, d), log, remove, swap):
            who = ask(ledger, "press_unsized(E9, I, %s, %s)" % (thing, d), log, remove, swap)[0]["bind"]["I"]
            found.append((e, "cannot tell", head + "\n  CANNOT TELL. Something does press it that way (%s), but the ledger does not say how strongly %s can press." % (who, who))); continue
        else:
            used = said
    ...
    for name, ids in events.items():
        ...
        for e in ids:
            ...
            for (pe, kind, _) in found:
                if pe == e and kind not in ("fine", "note", "not checked", "cannot tell"): f.add("a law is broken"); f.add("which slot: " + kind)
        feats[name] = f
...
def report(path, log):
    ...
    if ab: out.append(ab + "\n")
    unknown_verbs = sorted({a["bind"]["V"] for a in ask(ledger, "no_shape(E, V, T)", log)})
    marks = [v["mark"] for v in meta["lines"].values()]
    out.append("GAUGE: %d lines said, %d filled in, %d usual case. Verbs with no shape: %s. Left in the bin: %s. Slowest question %.2f seconds%s." % (marks.count("said"), marks.count("filled in"), marks.count("usual case"), ", ".join(unknown_verbs) or "none", "; ".join(meta["leftover"]) or "nothing", ask.slowest, "; SOMETHING RAN OUT OF TIME" if ask.timed_out else ""))
    return "\n".join(out)
