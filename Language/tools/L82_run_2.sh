...
set -u
[ $# -eq 3 ] || { echo "usage: L82_run_2.sh CORPUS_DIR OUT_DIR PAIRS_FILE" >&2; exit 2; }
ROOT="$(pwd)"; T="$ROOT/Language/tools"; RIG="$ROOT/Language/rigs/rig 1 - arguments"
[ -f "$T/ask_model_2.py" ] && [ -d "$RIG/patched" ] || { echo "run from the repository root" >&2; exit 2; }
...
for pid in $(pgrep -f 'ask_model(_2)?\.py|translate_via_api(_2)?\.py|L82_run(_2)?\.sh' 2>/dev/null); do
...
def call(job):
    code, up = job
    r = subprocess.run([sys.executable, os.path.join(T, "ask_model_2.py"), "atria", "--user", up, "--out", os.path.join(OUT, "reader"), "--tag", code, "--max-tokens", "30000", "--temperature", "0.1"], capture_output=True, text=True)
    rc = r.returncode
    if rc == 0:
...
def call(job):
    tag, up = job
    r = subprocess.run([sys.executable, os.path.join(T, "ask_model_2.py"), "mimo", "--user", up, "--out", os.path.join(OUT, "prose_reader"), "--tag", tag, "--max-tokens", "30000", "--temperature", "0.1"], capture_output=True, text=True)
    rc = r.returncode
    if rc == 0:
