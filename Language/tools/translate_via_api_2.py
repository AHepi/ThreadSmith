...
Arm B run; this version calls ask_model_2.py (streamed, silence timeout 600 s, six attempts) with a completion cap per provider (Atria 65,536, the most its API accepts; Mimo 100,000), because
...
CAP = {"atria": 65536, "mimo": 100000}   # completion caps per provider: Atria rejects any max_tokens above 65536 (400, 22 September, second run); Mimo accepts 100000
...
def main():
    ...
    for attempt in range(1, attempts + 1):
        ...
        rc = subprocess.run([sys.executable, os.path.join(HERE, "ask_model_2.py"), provider, "--user", user_path, "--out", out_dir, "--tag", tag, "--max-tokens", str(CAP[provider]), "--temperature", "0.2", "--idle", "600", "--attempts", "6"]).returncode
