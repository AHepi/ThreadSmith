...
def check(ledger_path, log):
    ...
    for case_name, its_lines in cases.items():
        ...
        if not fresh:
            report.append(("INSIDE '%s' (a told world, looked at alone): no contradiction.%s" if told else "UNDER THE SUPPOSITION '%s': no new contradiction.%s") % (case_name, " The contradiction listed above was there already and is not this supposition's doing." if seen else ""))
