...
## Current state
- **Authority: [W8, the model of an LLM agent in the language of the semantics, file 11](<authority/W8 Model - an LLM agent in the language of the semantics, file 11 (frozen).md>), frozen 22 September 2026** (log W6, decisions W3 and W4). Thirty-four parts in three slices (the call; the arrangement; the practice and the map), each on quoted sentences of file 11 with a file mark set by program, each with one of the eight marks and what would count against it; a table placing all twenty-one operations of Part XIV's dependence order (at one call, across calls, only with a record or a harness, never); seventy-seven word-list rows; the eleven notions of H69 each given a row or declined. Built by three Opus 5 builders and judged by two, all armed with the hard-to-vary skill; the judges' twenty-nine forced edits are written into the program that regenerates it. Four parts are marked unknown because the two judges marked them differently (plan W6's rule) and wait on a third reading. No reader has run under it; phase 4 is its first test. [W6 Results](<results/W6 Results - phase 1, the model built by three Opus 5 builders and judged by two; W8 frozen.md>) ticks the nine predictions of [plan W6](<tests/W6 Plan - phase 1, the model of an LLM agent in the language of file 11, built with five Opus 5 agents.md>).
...
authority/     W8, the model (frozen, current); W7, its draft
