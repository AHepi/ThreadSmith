...
**None of them has been read by any run in the record.** Checked document by document against `HV Skill/rigs/plan 49 rig - DeepSeek on outside papers/sources.json` (49 sources), against HV file 50 (the same 49, with their trims), and against HV file 43 (twenty-five passages written by Claude, no outside documents at all). No address and no document here appears in any of the three. Two documents in the record's *sources* stand next to documents here, both philosophy and both now in the arms: the old rig's P7 was Plato's *Meno* at Project Gutenberg's epub 1643, Benjamin Jowett's translation, and W7 here is Plato's *Euthyphro* in Jowett's translation at epub 1642; its P2 was David Hume's *Enquiry Concerning Human Understanding*, Section VII, at epub 9662, and W8 here is Hume's *Dialogues Concerning Natural Religion*, Part 2, at epub 4583. Each pair is a different work at a different address, so the claim above stands and no run has read W7 or W8; what is shared is the author, and for W7 the translator as well (one P7 report in the record is headed "Plato's *Meno* (Jowett translation)" and another marks Jowett's introduction rather than the dialogue, which is how the translation is known here), which is a nearer neighbour than a famous name in a reply and is named here rather than left out. Two names do appear in the record's *replies*: "Hilbert" in the DeepSeek returns on C1, C2, C3 and "Michelson" in one return on F5. A reply that mentions a name is not a run that read the document, and W1 and W4 are new documents; but a reader may carry something about either from its weights, which is true of every famous document and is recorded here rather than left out.
