...
- **Project.** One folder at the root, with the standard shape (see [START-HERE.md](START-HERE.md)). Five exist; each is separate, with its own authority, record, numbering and next step. Work on a project touches only its own folder; the root is the one shared surface.
